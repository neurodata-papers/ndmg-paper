...
dsets = ["NKI1", "KKI2009", "HNU1", "BNU1", "SWU4"]

#%%
# Grab dictionary of {'dset1': [files], 'dset2': [files2], ...}
fs = OrderedDict()
for dset in dsets:
    p = Path("data/multisite") / Path(dset)
    fs[dset] = [str(i.resolve()) for i in list(p.iterdir())]
# for idx, dset in enumerate(dsets):
#     fs[dset] = [
#         root + "/" + fl
#         for root, dirs, files in os.walk(os.path.join(dpath, dset))
#         for fl in files
#         if fl.endswith(".pkl") and "summary" not in fl
#     ]
#%%
labs = [
...
# get list of number of subjects in each dataset
nsubs = []
